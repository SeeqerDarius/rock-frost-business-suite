import { LayoutGrid, Building2, CreditCard, Blocks, Activity, Inbox, Settings, LifeBuoy } from "lucide-react";
import type { ModuleNavItem } from "@/types/module";
import { getPlatformUnreadCount } from "@/lib/support/service";

/** Platform-scope navigation — for Rock Frost operators managing the SaaS across all tenant organizations. */
export async function getPlatformNavigation(): Promise<ModuleNavItem[]> {
  const unreadSupportCount = await getPlatformUnreadCount();
  return [
    ...
    { label: unreadSupportCount > 0 ? `Support (${unreadSupportCount})` : "Support", href: "/app/platform/support", icon: <LifeBuoy className="size-4" /> },

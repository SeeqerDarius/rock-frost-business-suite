...
export function PwaProvider({ workspace, children }: PwaProviderProps) {
  ...
  return (
    <>
      {showBadge ? (
        // Centered at the top of the viewport rather than the top-right
        // corner, so it never overlaps the header's own right-side controls
        // (module launcher, account avatar) regardless of what else is
        // showing there (the trial/subscription pill sits further left of
        // center - see app/app/layout.tsx).
        <div className="fixed left-1/2 top-3 z-50 flex max-w-[90vw] -translate-x-1/2 items-center gap-2 rounded-full border bg-background/95 px-3 py-1.5 text-xs font-medium shadow-sm backdrop-blur" role="status" aria-live="polite">

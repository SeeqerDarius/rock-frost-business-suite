...
export default function OfflinePage() {
  ...
  return (
    <main id="main-content" className="mx-auto flex min-h-screen max-w-2xl flex-col justify-center gap-6 px-6 py-12">
      ...
      {workspaces.length ? <section className="space-y-3" aria-labelledby="cached-workspaces"><h2 id="cached-workspaces" className="text-lg font-semibold">Authorized cached workspaces</h2>{workspaces.map((workspace) => <article key={workspace.partitionKey} className="rounded-lg border p-4"><p className="font-medium">{workspace.organizationName}</p><p className="text-sm text-muted-foreground">Last synchronized {new Date(workspace.capturedAt).toLocaleString()}. Read-only module navigation: {workspace.moduleKeys.join(", ") || "none"}.</p></article>)}</section> : <p className="rounded-lg border p-4 text-sm">No unexpired workspace snapshot is available on this device.</p>}

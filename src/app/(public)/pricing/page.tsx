...
export default async function PricingPage() {
  ...
  return <>
    ...
    <section className="public-section-tint"><div className="mx-auto max-w-6xl px-6 py-16">
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">{modulePrices.map((price) => <Card id={price.moduleKey === "fleet" ? "fleet-pricing" : undefined} key={price.moduleKey} className="flex scroll-mt-24 flex-col"><CardHeader><CardTitle>{names.get(price.moduleKey)}</CardTitle><CardDescription className="space-y-2"><span className="block">{moduleValue[price.moduleKey] ?? "A focused operating workspace for your organization"}</span><span className="block">From <span className="text-2xl font-semibold text-foreground">{formatGhs(price.monthlyGhs)}</span> / month</span></CardDescription></CardHeader><CardContent className="mt-auto space-y-4"><div className="space-y-2 text-sm"><p className="flex items-center gap-2"><Users className="size-4 text-primary" />{price.includedSeats} user seats included</p><p className="flex items-center gap-2"><Check className="size-4 text-primary" />{formatGhs(price.annualGhs)} when paid annually</p><p className="flex items-center gap-2"><Check className="size-4 text-primary" />Extra seats: {formatGhs(price.additionalSeatGhs)} / month</p></div>{price.moduleKey === "fleet" ? <Button className="w-full" nativeButton={false} render={<Link href="/contact?intent=demo&module=fleet" />}>Book a Fleet walkthrough</Button> : null}<Button className="w-full" variant="outline" nativeButton={false} render={<Link href={`/subscribe?type=module&product=${price.moduleKey}`} />}>Subscribe to this module</Button></CardContent></Card>)}</div>
    </div></section>

...
export async function recordFolioPaymentAction(formData: FormData) { const path = "/app/hotel/folios"; const tenant = await authorize(PERMISSIONS.HOTEL_FOLIOS_MANAGE, path); const parsed = parseWithSchema(z.object({ folioId: cuid, amount: moneyAmountPositive, method: paymentMethods, reference: shortText.nullable() }), { folioId: clean(formData.get("folioId")) ?? "", amount: clean(formData.get("amount")), method: clean(formData.get("method")), reference: clean(formData.get("reference")) }); if (!parsed.success) redirect(`${path}?error=invalid`); const { folioId, ...data } = parsed.data; const payment = await recordHotelPayment(tenant.organizationId, folioId, data); await postModuleRevenue(tenant.organizationId, { sourceModule: "hotel", sourceType: "HOTEL_PAYMENT", sourceId: payment.id, postingPurpose: "COLLECTED", amount: payment.amount.toString(), entryDate: payment.receivedAt, description: `Hotel payment received: receipt ${payment.receiptNumber}`, createdById: tenant.userId }); revalidatePath(path); redirect(`${path}?saved=1`); }

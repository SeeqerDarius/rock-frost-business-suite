...
export default async function HotelLayout({ children }: { children: React.ReactNode }) {
  ...
  return <AppShell sectionLabel="Hotel Management" moduleKey="hotel" navigation={hotelNavigation} moduleSections={getFullModuleNavigation(tenant)} enabledModuleKeys={tenant.accessibleModuleKeys} organization={{ organizationId: tenant.organizationId, memberships: tenant.memberships }}>{children}</AppShell>;
}

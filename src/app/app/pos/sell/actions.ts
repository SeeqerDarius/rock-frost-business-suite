...
const saleSchema = z.object({
  ...
  clientRequestId: optional(shortText),
  occurredAt: optional(shortText),
});
...
export type CompleteSaleResult =
  | { ok: true; saleNumber: string; suspended: boolean }
  | { ok: false; error: string };

/**
 * RPC-style, not a native form action: called via startTransition from
 * sale-cart.tsx rather than a <form action>, so the client can tell a real
 * network failure (queue the sale in the offline-sync queue,
 * src/app/app/pos/sell/offline-queue.ts) apart from a validation error
 * (show it inline), and so a completed sale doesn't force a full-page
 * redirect/reload. `clientRequestId`/`occurredAt` are only ever set when
 * this call is an offline-queue sync replay.
 */
export async function completeSale(formData: FormData): Promise<CompleteSaleResult> {
  const tenant = await requireModuleAccess("pos");
  if (!hasPermission(tenant, PERMISSIONS.POS_SALES_MANAGE)) return { ok: false, error: "forbidden" };
  const parsed = parseWithSchema(saleSchema, {
    ...
    mode: clean(formData.get("mode")),
    clientRequestId: clean(formData.get("clientRequestId")),
  ...
  try { rawLines = JSON.parse(clean(formData.get("lines"))); rawPayments = JSON.parse(clean(formData.get("payments"))); } catch { return { ok: false, error: "invalid-line" }; }
  ...
  if (!linesParsed.success || !paymentsParsed.success || (parsed.data.mode === "COMPLETED" && paymentsParsed.data.length === 0)) return { ok: false, error: "invalid-line" };
  const occurredAt = parsed.data.occurredAt ? new Date(parsed.data.occurredAt) : null;
  if (parsed.data.occurredAt && (!occurredAt || Number.isNaN(occurredAt.getTime()))) return { ok: false, error: "invalid-line" };
  const session = await getServerAuthSession();
  let sale;
  try {
    sale = await createSale(tenant.organizationId, {
    ...
    await logAuditEvent({ organizationId: tenant.organizationId, userId: session?.user?.id ?? null, module: "pos", action: parsed.data.mode === "SUSPENDED" ? "pos.sale.suspend" : "pos.sale", entityName: "PosSale", entityId: sale.id, metadata: { saleNumber: sale.saleNumber, total: Number(sale.total), lineCount: linesParsed.data.length, offlineSync: Boolean(parsed.data.clientRequestId) } });
    ...
    if (parsed.data.mode === "COMPLETED") {
      await postModuleRevenue(tenant.organizationId, {
        ...
        entryDate: sale.occurredAt ?? sale.createdAt,
        description: `POS sale ${sale.saleNumber}`,
        createdById: session?.user?.id ?? null,
      });
    }
  } catch (error) {
    if (error instanceof InsufficientStockError) return { ok: false, error: "insufficient-stock" };
    if (error instanceof SaleStateError) return { ok: false, error: "no-open-session" };
    if (error instanceof InvalidSaleInputError) return { ok: false, error: "invalid-line" };
  ...
  return { ok: true, saleNumber: sale.saleNumber, suspended: parsed.data.mode === "SUSPENDED" };
}

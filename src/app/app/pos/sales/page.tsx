        ...
        <Table>
          <TableHeader>
            <TableRow>
              ...
              <TableHead>Total ({tenant.organization.currency})</TableHead>
              ...
              <TableRow key={sale.id}>
                ...
                {canManage || canReturn ? (
                  <TableCell className="text-right">
                    {sale.status === "SUSPENDED" && canManage ? <EntityDialog trigger={<Button size="sm" variant="ghost">Resume</Button>} title={`Resume ${sale.saleNumber}`} action={resumeSale} submitLabel="Complete sale"><input type="hidden" name="saleId" value={sale.id} /><div className="space-y-2"><Label>Payment method</Label><select name="method" className="h-8 w-full rounded-lg border bg-background px-2"><option value="CASH">Cash</option><option value="CARD">Card</option><option value="MOBILE_MONEY">Mobile money</option><option value="OTHER">Other</option></select></div><div className="space-y-2"><Label>Amount ({tenant.organization.currency})</Label><Input name="amount" type="number" step="0.01" defaultValue={Number(sale.total).toFixed(2)} /></div></EntityDialog> : null}

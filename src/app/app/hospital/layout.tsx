...
export default async function HospitalLayout({ children }: { children: React.ReactNode }) {
  ...
  return <AppShell sectionLabel="Hospital Management" moduleKey="hospital" navigation={hospitalNavigation} moduleSections={getFullModuleNavigation(tenant)} enabledModuleKeys={tenant.accessibleModuleKeys} organization={{ organizationId: tenant.organizationId, memberships: tenant.memberships }}>{children}</AppShell>;
}

...
export default async function FleetLayout({ children }: { children: React.ReactNode }) {
  ...
  const routeAccess: Array<[string, boolean]> = [
      ...
      ["/app/fleet/maintenance", hasPermission(tenant, PERMISSIONS.FLEET_MAINTENANCE_MANAGE) || hasPermission(tenant, PERMISSIONS.FLEET_VIEW) || hasPermission(tenant, PERMISSIONS.FLEET_INVESTOR_VIEW)],

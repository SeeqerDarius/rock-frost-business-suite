...
export default async function AccountingOverviewPage() {
  ...
  const stats = [
    { label: "Cash balance", value: formatMoney(summary.cashBalance, tenant.organization.currency), description: "Combined balance across cash accounts", icon: <Wallet className="size-4" />, href: "/app/accounting/accounts" },
    ...
    { label: "Net income", value: formatMoney(summary.netIncome, tenant.organization.currency), description: "Revenue less expenses for the period", icon: <TrendingUp className="size-4" />, href: "/app/accounting/reports" },
  ];

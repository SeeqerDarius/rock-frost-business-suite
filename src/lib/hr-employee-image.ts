export const MAX_HR_EMPLOYEE_PHOTO_BYTES = 1024 * 1024;
...
export async function hrEmployeePhotoData(file: File) {
  if (!file.size) return null;
  if (file.size > MAX_HR_EMPLOYEE_PHOTO_BYTES || !IMAGE_TYPES.has(file.type)) throw new Error("invalid-employee-photo");
  const bytes = new Uint8Array(await file.arrayBuffer());
  if (!hasValidSignature(bytes, file.type)) throw new Error("invalid-employee-photo");
  return `data:${file.type};base64,${Buffer.from(bytes).toString("base64")}`;
}

export function parseHrEmployeePhoto(data: string) {

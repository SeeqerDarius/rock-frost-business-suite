...
function modelsForScope(scope: BackupModule | "all") {
  const prefixes = scope === "all" ? Object.values(MODEL_PREFIXES).flat() : MODEL_PREFIXES[scope];
...
export async function buildTenantBackup(organizationId: string, tenantCode: string, scope: BackupModule | "all") {
  const models: TenantBackup["models"] = {};
  await Promise.all(modelsForScope(scope).map(async (model) => {
...
export function validateTenantBackup(value: unknown, organizationId: string, tenantCode: string): value is TenantBackup {
  ...
  const allowedModels = new Set(modelsForScope(backup.scope).map((model) => model.name));
...
export async function mergeTenantBackup(backup: TenantBackup) {
  const selected = modelsForScope(backup.scope).filter((model) => backup.models[model.name]);
  let restored = 0;

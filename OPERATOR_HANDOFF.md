...
- Replaced the atmospheric radial glows, fixed background effects, glass blur, and decorative CTA circle with a restrained corporate background, solid panels, subtle borders, and limited shadow.
- Removed the prohibited em dash character from the tracked repository, including customer-facing copy and source comments. `AGENTS.md` now prohibits it, and `test/editorial-punctuation.test.ts` prevents it from returning to shipped source.

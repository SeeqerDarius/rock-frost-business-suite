...
- Release: to be committed and pushed to this branch, then merged to `main` and deployed with the operator's authorization, matching the pattern already established this session for the earlier desktop bug-fix branches.
- Pre-existing files preserved: `Rock-Frost-Project-Status-Report.pdf`, `output/`, and `tmp/` remain untracked and untouched by this branch.

...
Local release-candidate validation: Prisma format, validation, and generation passed. TypeScript passed. ESLint passed. The focused suite passed 4 files and 27 tests. The full mocked suite passed 82 files and 483 tests. The Next.js 16.2.12 production build compiled successfully and generated all 208 static pages. `git diff --check` passed, and no newly added line contains the prohibited em dash character. The two new real-PostgreSQL suites cover Fleet payment evidence, daily Work & Pay periods, approval effects, HR enablement, external-role exclusions, tenant scoping, and concurrent idempotency. Local execution is unavailable because this workstation has neither `TEST_DATABASE_URL` nor Docker; the repository CI disposable-PostgreSQL job is therefore a mandatory gate before production deployment. Release commit, CI result, migration deployment, production deployment, health checks, route probes, and error-log verification will be appended after the release completes.

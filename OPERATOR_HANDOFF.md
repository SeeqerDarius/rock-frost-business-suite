...
- Release: implementation on `agent/claude-desktop-cors-fix`, to be merged into `main` alongside the outstanding `0.2.3` CSP fix from `agent/claude-desktop-fetch-binding-fix` (commit `ac1ffb1`, not yet on `main` as of this entry), pushed, and deployed, then re-verified live with the same `curl -X OPTIONS` check now returning `Access-Control-Allow-Origin: https://tauri.localhost`. See the following entry for the merged release record.
- Pre-existing files preserved: `Rock-Frost-Project-Status-Report.pdf`, `output/`, and `tmp/` remain untracked and untouched by this branch.

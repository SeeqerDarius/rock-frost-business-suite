...
## Build result (Hardening Pass 2)

**Passed.** `npm run lint` clean, `npx tsc --noEmit` clean, `npx prisma validate` succeeds, `npm run test` — 45/45 passing across 5 files (27 from Pass 1 + 18 new), `npm run build` succeeds — 101 routes (unchanged; this pass added no pages, only mutation logic).

## Known issues / deliberate gaps (current)

- **Pass 3 not started**: invitation redesign (still activates every `INVITED` membership on one accept, still replaces an existing user's password), formal Zod runtime validation (still none — Server Actions rely on `String()`/`parseInt`/`parseFloat`/`new Date()`), full line-by-line IDOR audit of CRM/HR/Fleet (flagged by the audit as likely-present-but-unconfirmed), Decimal-precision arithmetic throughout Accounting/Payroll/Installment (still converts to JS `Number`), reproducible seeding/CI (`.env.example`, committed RBAC seed script, CI workflow, Node version pin).
- **Documented narrow residual concurrency races** (none corrupt the primary financial figure, all in `docs/HARDENING_PLAN.md`'s "Documented residual concurrency risks" section): Accounting's invoice-payment remaining-balance guard reads a pre-transaction snapshot; Installment's payment status/clamp write has a narrow three-way-interleaving window. Closing these fully needs either `SELECT ... FOR UPDATE` row locking or serializable transaction isolation — judged disproportionate to hold up this pass.
- **Installment's `updatePayment()` and `applyCreditToAccount()`** retain the same read-then-write race class fixed elsewhere in that module this pass — narrower in practice (both need two specific actions racing on the *same* account in a short window) but not yet closed.
...
- Every gap carried forward from Pass 1 and earlier remains true and is unaffected by this pass: no data-level scoping in any module, several modules not yet linked to Accounting, POS's three-fixed-line UI, Analytics' lack of time-series drilldown, Fleet's missing owner portal/file uploads, no branch-level enforcement, no public self-registration, unset `RESEND_API_KEY`, functionally inert (single-org) organization switcher. Full list in `docs/HARDENING_PLAN.md`.

## Next recommended step

Get explicit direction before starting Pass 3, per the same review-checkpoint discipline used between Pass 1 and Pass 2. Suggested order per `docs/HARDENING_PLAN.md`: invitation redesign first (it's a confirmed, real security gap — one org's invite can activate access to another), then formal Zod validation across every mutating Server Action (large and cross-cutting, worth doing once rather than piecemeal), then the remaining CRM/HR/Fleet IDOR audit, then Decimal-precision hygiene and reproducible seeding/CI.
...
See "Files changed (Hardening Pass 2...)," "Summary of what was done (Hardening Pass 2)," "Build result (Hardening Pass 2)," "Known issues / deliberate gaps (current)," and "Next recommended step" above — kept in the current-state sections rather than duplicated here, since this is the most recent entry. Full plan and Pass 3 scope in `docs/HARDENING_PLAN.md`.

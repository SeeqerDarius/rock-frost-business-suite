...
- Remaining risk: no live browser verification - this environment has no credentials for a real logged-in organization session, and the School pages sit behind auth against the real production database, so a dev-server click-through wasn't possible here. The operator has not seen any of these UI changes rendered. CI's integration-suite result for the new migration has not yet been confirmed.

...
- Release: pending commit, CI, push, production deployment, health check, and log review. Production `vercel-build` runs the idempotent permission seed before compiling, so `org.data.export` will exist before the new routes are served.

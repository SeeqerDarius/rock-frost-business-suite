...
- **Installment's `updatePayment()`/`applyCreditToAccount()`** (carried forward from Pass 2) retain the same narrower read-then-write race class fixed elsewhere. **Still current** (Pass 3c improved `applyCreditToAccount()`'s arithmetic precision but did not close this specific race).

...
- **Known limitation, not tested live**: full end-to-end verification (an actual AI reply appearing in a real conversation) requires a real `ANTHROPIC_API_KEY`, which is not configured in this environment (local `.env` has it empty, matching what's presumably also true in production). Every code path was verified with a mocked Anthropic client; I did not and will not enter the user's real credentials anywhere to test this live myself. With the key unset, the feature is fully inert — the human-only support flow is completely unaffected, confirmed by the full test suite and the live dev-server check above.

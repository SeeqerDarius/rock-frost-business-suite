...
**All sixteen product phases are feature-complete (see `docs/DEVELOPMENT_ROADMAP.md`), but a 2026-07-20 full-project audit found the platform is only production-ready for controlled/internal use, not external multi-tenant onboarding or real financial operations — see `docs/HARDENING_PLAN.md`.** The project is now in a dedicated **production-hardening track**, run in numbered passes rather than feature phases. **Hardening Pass 1 (tenant guard, session revocation, dashboard permission leak, and the Administration/Projects/Payroll IDOR paths) is complete** — see `docs/HARDENING_PLAN.md` for full detail and Pass 2's scope (remaining IDOR paths, financial/inventory transaction atomicity, invitation redesign, runtime validation, broader test/CI coverage). Billing/Subscriptions remains an explicit "Planned — requirements not yet defined" placeholder; it was never implemented scope and is not part of the hardening track.
...
## Build result (Hardening Pass 1)

**Passed.** `npm run lint` clean, `npx tsc --noEmit` clean, `npx prisma validate` succeeds, `npm run test` — 27/27 passing across 4 files, `npm run build` succeeds — 101 routes (unchanged; this pass added no pages). `vercel --prod` confirmed `READY`.

## Known issues / deliberate gaps (current)

- **Pass 2 is not started**: remaining confirmed IDOR paths in Procurement (vendor/request/item/warehouse ids), Accounting (manual journal account ids), Inventory (source/destination warehouse ids), and POS (register id on session open) — all overlap with the financial/inventory atomicity rework below, so fixing the IDOR alone without the transaction work would be incomplete. See `docs/HARDENING_PLAN.md`'s Pass 2 section for the full list.
- **Financial/inventory transaction integrity is not yet fixed** — POS negative-quantity/non-atomic sale+stock-deduction, Inventory read-then-write races, Procurement receiving order-update ordering, Accounting double-posting/void-without-reversal/Decimal-to-Number conversion, Payroll duplicate-run processing, Installment concurrent-payment races. All confirmed by the audit, all deferred to Pass 2 per explicit instruction to stop before this broader refactor.
- **Invitation design is still not tenant-specific** — accepting one invite token still activates every `INVITED` membership for that user, and still unconditionally replaces an existing active user's password. Confirmed real, requires a schema change (a proper `Invitation` model bound to one membership) that's correctly out of scope for a same-pass IDOR fix. See `docs/HARDENING_PLAN.md`'s invitation-redesign section.
- **No formal runtime validation (Zod)** — Server Actions still rely on `String(...).trim()`/`parseInt`/`parseFloat`/`new Date(...)`/enum casts, none of which validate untrusted input at runtime. Zod is an installed but unused dependency. Large, cross-cutting; not started.
- **`src/app/app/(overview)/modules/page.tsx` still uses `enabledModuleKeys`** (not `accessibleModuleKeys`) for its own "Open" button — deliberately left as-is this pass since it's a UX inconsistency (a dead-end link, independently blocked by the target module's own layout guard), not a data leak. One-line follow-up.
- **No CI workflow, `.env.example`, Node version pin, or committed RBAC/module seed script** — reproducible-setup work, Pass 2+.
- Every gap carried forward from Phase 16 and earlier remains true and is unaffected by this pass: no data-level scoping in any module, POS/Payroll/Procurement/HR/Accounting not yet linked to Accounting where noted, POS's three-fixed-line UI, Analytics' lack of time-series drilldown, Fleet's missing owner portal/file uploads, no fuzzy duplicate-detection, no branch-level enforcement, no public self-registration, unset `RESEND_API_KEY`, functionally inert (single-org) organization switcher. Full list in `docs/HARDENING_PLAN.md`.

## Next recommended step

Begin Pass 2 per `docs/HARDENING_PLAN.md`: the financial/inventory transaction-integrity rework (POS, Inventory, Procurement, Accounting, Payroll, Installment) and its overlapping remaining IDOR paths, together — since fixing the IDOR gaps in those modules in isolation would be incomplete without also fixing the atomicity issues in the same code. After that: the invitation redesign (new `Invitation` model), then formal Zod validation across every mutating Server Action, then broader test/CI coverage. Get explicit direction before starting Pass 2 given its size, per the same "stop for review" instruction that ended Pass 1 here.
...
See "Files changed (Hardening Pass 1...)," "Summary of what was done (Hardening Pass 1)," "Build result (Hardening Pass 1)," "Known issues / deliberate gaps (current)," and "Next recommended step" above — kept in the current-state sections rather than duplicated here, since this is the most recent entry. Full plan and Pass 2 scope in `docs/HARDENING_PLAN.md`.

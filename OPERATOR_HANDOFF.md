...
## 2026-08-20: Fix — Driver role assignment never created a matching FleetDriver

- The user asked why someone assigned the Driver role from Administration didn't show up on `/app/fleet/drivers`. Root cause: assigning that role only ever changed the member's login permissions (`fleet.driver.self_service`); it never touched the separate `FleetDriver` table the Drivers page lists and driver self-service pages resolve "me" against (`FleetDriver.userId`). A Fleet manager had to remember a second, disconnected manual step — create the roster row, then link it to the person's login via a dropdown that (also confirmed) lists every active org member, not just Driver-role holders. Until that happened, the person's own self-service pages resolved nothing either.
- Fix, confirmed with the user before implementing: `ensureFleetDriverForUser(tx, organizationId, userId)` in `src/modules/fleet/service.ts` — idempotent (relies on the existing `@@unique([organizationId, userId])` constraint), creates a `FleetDriver` row seeded from the user's name/email. Called from every place a member ends up holding a driver-permission role while active: `changeMemberRole` (`src/app/app/(overview)/administration/actions.ts`), and both invitation-acceptance paths (`acceptInvitationNewUser`/`acceptInvitationExistingUser` in `src/lib/auth/invitations.ts`). An INVITED member's driver record is deliberately deferred to acceptance, not created at invite-time.
- Also added a lazy backfill (`backfillMissingFleetDrivers`, called from `listFleetDrivers`) so members already holding the role before this fix shipped self-heal onto the roster the next time anyone opens the Drivers page — the same self-healing-on-read pattern `ensureDefaultAccounts()` already established for Accounting. No manual data migration needed for existing orgs.
- Important files: `src/modules/fleet/service.ts`, `src/app/app/(overview)/administration/actions.ts`, `src/lib/auth/invitations.ts`, `docs/DEVELOPMENT_ROADMAP.md`.
- Validation: `npx tsc --noEmit` passed with zero errors; `npm run lint` passed; `npm run build` compiled all routes successfully. New tests: `test/fleet-driver-role-sync.test.ts` (mocked-db: creation, idempotency, the name-fallback-to-email case, the backfill query shape, and that it's a no-op when nothing is missing) and `test/integration/tenant-isolation/fleet-driver-role-sync.test.ts` (real Postgres: `ensureFleetDriverForUser` creates exactly one row and stays idempotent on a second call; `listFleetDrivers` backfills a member holding the seeded "Driver" role and does *not* create one for a member holding a role without that permission). Full mocked suite: 79 files, 527 tests, all passing. No schema change, so no migration to verify in CI's integration job — the integration test exercises real Postgres data, not schema.

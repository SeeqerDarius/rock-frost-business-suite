...
## 2026-08-21: Cross-module revenue integration (branch `agent/claude-accounting-integration`, based on `codex/accounting-foundation`, NOT merged, NOT deployed)
...
## 2026-08-20: Accounting posting foundation

- Scope: immutable organization-scoped journal posting numbers, idempotent source-module posting identity, accounting period close/reopen controls, and reversal-based correction. New tenant UI: `/app/accounting/periods`; journal entries now show posting number/status and authorized users can create a compensating reversal.
- Schema and migration: `AccountingPeriod`, `AccountingPeriodStatus`, `AccountingJournalStatus`, plus `AccountingJournalEntry.postingNumber`, `postingPurpose`, `status`, and `reversalOfId`. Migration: `20260820231500_accounting_posting_foundation`. Existing journal rows are backfilled deterministically per organization before the posting-number column becomes required.
- Security: new `accounting.periods.manage` and `accounting.journals.reverse` permissions, organization-scoped service queries, closed-period guard at the shared journal choke point, and tenant-scoped audit events for period changes and journal reversals.
- Integration contract: `postSourceJournalEntry()` is the only supported posting boundary for other modules. Its organization/source/purpose identity makes retries idempotent. Direct writes from other modules remain prohibited.
- Validation: Prisma format/validate/generate passed; TypeScript passed with `--incremental false`; ESLint passed; the full mocked suite passed 83 files and 558 tests; the production build compiled 206 routes including `/app/accounting/periods`; `git diff --check` passed. Focused Accounting regression coverage passed 4 files and 46 tests. The module-access sweep was updated from 102 to 103 pages and 53 to 54 action files for the new guarded route. `TEST_DATABASE_URL` is not configured, so the migration and real PostgreSQL integration/concurrency suite remain required in CI before merge or deployment.
- Build isolation: the stopped Copilot worktree lives under `worktrees/` inside the repository. Root TypeScript and ESLint configuration now exclude `worktrees/**` and `.claude/worktrees/**`, preventing duplicate agent checkouts from being compiled as production source. Each branch remains responsible for validating inside its own worktree.
- Concurrent work: Claude's isolated clinical branch and the stopped Copilot worktree were not merged. Copilot's schema-only commit contained placeholder tests and no executable migration, so it was rejected as production-incomplete.

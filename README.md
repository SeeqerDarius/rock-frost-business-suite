...
The controlled offline desktop foundation supports tenant-scoped device activation, revocation, bounded reference snapshots, idempotent mutation upload, and explicit conflict handling for selected Fleet, Installment, Inventory, and POS create workflows. The Windows client lives under `apps/desktop`, uses Tauri with encrypted local SQLite and Windows Credential Manager, and has passed native compilation plus NSIS/MSI packaging. Version `0.2.0` adds authenticated, user-confirmed automatic updates through public GitHub releases. High-risk approvals, refunds, accounting posting, payroll, HR termination, pharmacy dispensing, and clinical workflows remain online-only. Windows Authenticode signing remains a distribution-hardening requirement while public test distribution continues. See `docs/OFFLINE_DESKTOP.md`.

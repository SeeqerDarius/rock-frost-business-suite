...
- **School Management:** tenant-scoped School staff invitation and access management, tabbed student and guardian administration with one unified admission form that requires atomic guardian selection or creation, duplicate prevention, editable guardian details, an explicit learner-profile action, a permission-aware 360-degree learner profile, validated and optimized passport photos, signed and revocable digital IDs with public QR verification and wallet-size PDF output, academic periods, tabbed classes and subjects, enrollment, phone-first one-tap attendance rosters with bulk Present and live summaries, conduct, fees and Accounting integration, examinations, grading with a Ghana (WASSCE/BECE) grading-scale preset and a ranking-gated exam broadsheet, timetables, transport, library, conditional Hostel integration, campus services, payroll integration, optional SMS notifications for attendance/fees/results and a self-service Parent/Student portal, both paid per-organization add-ons a platform operator grants individually, and filterable attendance and collection dashboards with trends and prior-period comparisons. Every tenant user's profile also shows an assigned-role quick guide and a reliable tour replay action.

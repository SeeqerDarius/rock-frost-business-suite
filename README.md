...
- **Accounting:** double-entry invoicing and expenses plus classified cash, bank and mobile-money accounts, locked opening-balance entries, a derived cashbook, period reconciliation history, imprest petty cash funds, immutable posting numbers, source-idempotent module posting, serialized period close/reopen controls, journal reversals, and a Statement of Financial Position report.
...
The controlled offline desktop foundation supports tenant-scoped device activation, revocation, bounded reference snapshots, idempotent mutation upload, and explicit conflict handling for selected Fleet, Installment, Inventory, and POS create workflows. The Windows client lives under `apps/desktop`, uses Tauri with encrypted local SQLite and Windows Credential Manager, and has passed native compilation plus NSIS/MSI packaging. Version `0.2.0` added authenticated, user-confirmed automatic updates through public GitHub releases. Version `0.2.1` fixes the startup readiness guard so a successfully rendered activation or workspace screen cannot be replaced by a stale timeout. High-risk approvals, refunds, accounting posting, payroll, HR termination, pharmacy dispensing, and clinical workflows remain online-only. Windows Authenticode signing remains a distribution-hardening requirement while public test distribution continues. See `docs/OFFLINE_DESKTOP.md`.

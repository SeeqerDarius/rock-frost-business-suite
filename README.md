...
The controlled offline desktop foundation supports tenant-scoped device activation, revocation, bounded reference snapshots, idempotent mutation upload, and explicit conflict handling for selected Fleet, Installment, Inventory, and POS create workflows. The Windows client lives under `apps/desktop`, uses Tauri with encrypted local SQLite and Windows Credential Manager, and has passed native compilation plus NSIS/MSI packaging. High-risk approvals, refunds, accounting posting, payroll, HR termination, pharmacy dispensing, and clinical workflows remain online-only. A trusted code-signing certificate and signed update channel are required before customer distribution. See `docs/OFFLINE_DESKTOP.md`.

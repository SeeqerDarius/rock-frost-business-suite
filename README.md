...
- **Inventory & Procurement:** tenant-isolated item/category/warehouse catalogues, optional item images and barcodes, controlled physical counts, immutable movements, multi-line requisitions, purchasing approvals, purchase orders, numbered goods receipts, supplier-invoice matching and maker-checker approval, due dates, partial supplier payments, outstanding payable balances, low-stock alerts, reporting, POS integration, and automatic Accounting liability and settlement journals when Accounting is active.
...
- **Fleet Management:** administrator-linked driver logins with assigned-vehicle-only dashboards and navigation, daily or weekly vehicle remittance obligations, driver-recorded cash, mobile money and bank payments, assigned-driver-linked daily or weekly Work & Pay contracts and instalments, duplicate-safe payment periods, explicit manager review with transactional verified-ledger updates and tenant audit recording, private maintenance-photo reporting, ownership history, a linked Vehicle Owner portal role, and reporting. See `docs/FLEET_MODULE_IMPLEMENTATION.md`.
- **Accounting:** double-entry invoicing and expenses, explicit customer receipt allocations and statements, plus classified cash, bank and mobile-money accounts, locked opening-balance entries, a derived cashbook, period reconciliation history, imprest petty cash funds, immutable posting numbers, source-idempotent module posting, serialized period close/reopen controls, journal reversals, and a Statement of Financial Position report. Fleet, Pharmacy, Hospital, POS, Installment, Hostel, Hotel, and School automatically post their own confirmed revenue here the moment money is collected, each into its own Revenue account, with a "Revenue by source" breakdown on the Reports page so a manager can see exactly which module and record every figure came from.
...
- `docs/AUTHENTICATION_AND_AUTHORIZATION.md` - real, enforced auth/RBAC (159 permission keys across sixteen modules)

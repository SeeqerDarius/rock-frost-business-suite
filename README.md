...
- **Accounting:** double-entry invoicing and expenses plus classified cash, bank and mobile-money accounts, locked opening-balance entries, a derived cashbook, period reconciliation history, imprest petty cash funds, immutable posting numbers, source-idempotent module posting, serialized period close/reopen controls, journal reversals, and a Statement of Financial Position report. Fleet, Pharmacy, Hospital, POS, Installment, Hostel, Hotel, and School automatically post their own confirmed revenue here the moment money is collected, each into its own Revenue account, with a "Revenue by source" breakdown on the Reports page so a manager can see exactly which module and record every figure came from.
...
- `docs/AUTHENTICATION_AND_AUTHORIZATION.md` - real, enforced auth/RBAC (158 permission keys across sixteen modules)

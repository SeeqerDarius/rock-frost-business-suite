...
describe("HR service — cross-tenant IDOR fixes", () => {
  ...
  it("createResumeEntry rejects an employeeId from another organization", async () => {
    mockDb.hrEmployee.findFirst.mockResolvedValue(null);
    await expect(
      hr.createResumeEntry(ORG, "emp-foreign", { title: "Should fail", type: "EXPERIENCE", dateStart: new Date() }),
    ).rejects.toThrow(hr.NotFoundError);
    expect(mockDb.hrResumeEntry.create).not.toHaveBeenCalled();
  });

  it("updateResumeEntry rejects an entry id from another organization", async () => {
    mockDb.hrResumeEntry.findFirst.mockResolvedValue(null);
    await expect(
...
describe("Fleet service — cross-tenant IDOR fixes and payment atomicity", () => {
  ...
  it("createFleetPayment rejects a maintenanceRequestId from another organization", async () => {
    mockDb.fleetMaintenanceRequest.findFirst.mockResolvedValue(null);
    await expect(
      fleet.createFleetPayment(ORG, {
        reference: "REF-1",
        date: new Date(),
        type: "MAINTENANCE",
        amount: "300.00",
        maintenanceRequestId: "request-foreign",
      }),
    ).rejects.toThrow(fleet.NotFoundError);

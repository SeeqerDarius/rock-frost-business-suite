...
describe("Hotel service — real tenant isolation and lifecycle guards",()=>{
  ...
  it("enforces property numbering and housekeeping settings across the stay lifecycle",async()=>{
    await hotel.upsertHotelSettings(orgA.organizationId,{propertyId:propertyA.id,timezone:"Africa/Accra",currency:"GHS",checkInTime:"15:00",checkOutTime:"10:00",taxRate:"12.5",serviceChargeRate:"5",allowOutstandingCheckout:false,reservationPrefix:"BOOK",folioPrefix:"BILL",receiptPrefix:"RCPT",orderPrefix:"FOOD",autoCreateCheckoutTask:true,housekeepingDueHours:2,requireHousekeepingInspection:true,smsNotificationsEnabled:false});

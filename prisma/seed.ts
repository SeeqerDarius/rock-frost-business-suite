/**
 * Idempotent platform bootstrap seed — safe to re-run (every write is an
 * upsert or an existence check first). Seeds exactly the platform-level
 * data a fresh database needs to be usable: every Permission row, every
 * system Role (with its permission grants), and every Module row (marked
 * ACTIVE, matching src/platform/modules/registry.ts).
 *
 ...
 * data, not platform bootstrap, and belongs in a separate onboarding flow
 * (creating an Organization + its first Organization Owner) rather than a
 * schema-level seed.
 *
 * Run with: npx tsx prisma/seed.ts
 *
 * The permission key list below is intentionally duplicated from
 ...
 * real resolvable package outside Next's own build; see vitest.config.ts's
 * alias workaround for the same issue), which a plain `tsx` execution can't
 * resolve. If you add a new permission key to permissions.ts, add it here
 * too — docs/HARDENING_PLAN.md's Pass 3c entry flags this as the one place
 * that needs to stay in sync by hand.
 */
...
function moduleRolePermissions(prefix: string, keys: (typeof ALL_PERMISSIONS)[number][]) {
...
const ROLE_PERMISSIONS: Record<string, string[]> = {
  ...
  "Fleet Manager": moduleRolePermissions("fleet", [
  ...
  "Hire Purchase Manager": moduleRolePermissions("hirepurchase", [
  ...
  "CRM Manager": moduleRolePermissions("crm", [
  ...
  "Inventory Manager": moduleRolePermissions("inventory", [
  ...
  "Accounting Manager": moduleRolePermissions("accounting", [
  ...
  "HR Manager": moduleRolePermissions("hr", [
    PERMISSIONS.HR_VIEW,
  ...
  "Procurement Manager": moduleRolePermissions("procurement", [
  ...
  "Payroll Manager": moduleRolePermissions("payroll", [
  ...
  "Analytics Manager": moduleRolePermissions("analytics", [
  ...
  "POS Cashier": moduleRolePermissions("pos", [
  ...
  "Projects Manager": moduleRolePermissions("projects", [
...
async function main() {
  ...
  for (const role of roles) {
    ...
    console.log(`Seeded ${keys.length} permission grants for role "${role.name}".`);
  }
  ...
  console.log(`Upserted ${MODULES.length} modules (all ACTIVE).`);
}

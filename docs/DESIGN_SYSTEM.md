...
- `ModuleShowcase` (`src/components/marketing/module-showcase.tsx`) renders an illustrative in-app preview on a module's public landing page (`/modules/[moduleKey]`), right after the hero. It looks up the module key in `MODULE_SHOWCASES` and renders nothing if that module has no showcase configured; as of 2026-08-18 all 15 catalogue modules have one. Each showcase is a plain `h2` (no eyebrow label, matching this page's other sections and the homepage's `Modules` section) followed by an overview panel plus 1-2 feature panels, built from the shared kit in `src/components/marketing/module-showcases/kit.tsx` (`BrowserFrame`, `StatTile`, `FieldBox`, `PanelHeader`, `PanelNote`, `ShowcaseSection`) so every module's mockup shares the same visual language. Content is hand-built with clearly fictional sample data, never a real organization's records, matching the disclosure convention `CustomerShowcase` already uses for its `isDemo` entries — `ShowcaseSection` renders that disclosure line automatically. Add a new module's showcase by writing a component with the kit and registering it in `MODULE_SHOWCASES`.

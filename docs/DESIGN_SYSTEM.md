...
- **Icon hover motion**: every `lucide-react` icon renders with a `lucide` class regardless of call site (verified directly against the package's own output), so `src/app/globals.css` gives every icon in the app a hover animation with one global rule rather than a per-component treatment. Scoped to `:is(button, a, [role="button"], summary):hover` so a purely decorative icon (one that isn't inside a real interactive control) never implies clickability it doesn't have, and excludes `.animate-spin` icons so the hover animation can't fight a spinner's own rotation. Don't add a second, one-off hover-animation rule to an individual icon or component — this is the one place that treatment lives.
  - The default treatment (everything except the two exceptions below) is a "draw itself in" reveal: on hover, each icon's own `<path>`/`<rect>`/`<circle>`/etc. retraces its real stroke outline via `stroke-dasharray`/`stroke-dashoffset` (a `lucide-draw-in` keyframe animation), with multi-part icons staggering each child by 40ms so they trace in sequence rather than all at once. Because it traces each icon's own actual geometry, the motion is inherently specific to that icon's shape without hand-authoring 160+ individual animations. The shared `100` dasharray/dashoffset constant was chosen by rendering every icon name this app actually imports and measuring every real sub-shape's `getTotalLength()` in a live browser — the longest observed was ~86.4 (`BookPlus`). Re-measure the same way (not a guess) before ever shrinking that constant: a value smaller than some icon's real path length renders that icon as a broken dashed line at rest, not solid. Short/simple icons (`Plus`, `X`, arrows — this app's highest-frequency controls) finish revealing almost immediately and read as a quick flick rather than a slow trace; that's a deliberate tradeoff of sharing one constant across every icon size, not a bug — it keeps frequently-hovered controls feeling snappy.

...
- **Icon hover motion**: every `lucide-react` icon renders with a `lucide` class regardless of call site (verified directly against the package's own output), so `src/app/globals.css` gives every icon in the app a gentle `scale(1.12)` on hover with one global rule rather than a per-component treatment. Scoped to `:is(button, a, [role="button"], summary):hover` so a purely decorative icon (one that isn't inside a real interactive control) never implies clickability it doesn't have, and excludes `.animate-spin` icons so the hover transform can't fight a spinner's own rotation. Don't add a second, one-off hover-scale rule to an individual icon or component — this is the one place that treatment lives.
  - A couple of icons get a bespoke, per-part animation instead of the blanket scale (opted out by name in that rule's selector), matching how sites like Resend animate individual icons rather than applying one generic zoom everywhere: `Mail` (`lucide-mail`) has its flap `<path>` lift and pop slightly on hover while the envelope body `<rect>` stays put; `BarChart3` (`lucide-chart-column`) has its three bar `<path>`s grow taller from their own baseline with a staggered wave, while the axis-frame `<path>` stays put. Both rely on lucide's actual internal path order for that specific icon (confirmed by rendering it to static markup, not guessed from its rendered shape) — re-verify that order the same way before touching either rule, since a lucide-react upgrade that reorders an icon's paths would silently break it. This treatment currently covers only these two icon families (the two the user explicitly asked to see animated this way); extending it to more icons is real per-icon design work, not a formula to blanket-apply.

...
- `ModuleShowcase` (`src/components/marketing/module-showcase.tsx`) renders an illustrative in-app preview on a module's public landing page (`/modules/[moduleKey]`), right after the hero. It looks up the module key in `MODULE_SHOWCASES` and renders nothing if that module has no showcase configured, so it's opt-in per module. Each showcase (e.g. `src/components/marketing/module-showcases/school.tsx`) is hand-built with the real UI kit tokens (`bg-card`, `ring-foreground/10`, `text-muted-foreground`) inside a small browser-chrome frame, using clearly fictional sample data, never a real organization's records, matching the disclosure convention `CustomerShowcase` already uses for its `isDemo` entries. Add a new module's showcase by writing its component and registering it in `MODULE_SHOWCASES`.

...
## Feedback, greetings and workspace moments

The authenticated workspace includes a persistent but quiet “Share feedback” action leading to a dedicated consent-aware form rather than an interrupting modal. The organization dashboard greets the user according to the organization timezone, with `Africa/Accra` as a safe fallback. A local, curated motivation card may appear after the workspace settles, no more than once every four hours per user; it auto-dismisses after ten seconds, can be closed immediately, avoids payment/account/support workflows and uses `motion-safe` animation utilities.

...
The authenticated root retains `app/loading.tsx` as its server-streaming fallback (`RfLoadingScreen`, `src/components/feedback/rf-loading-screen.tsx`) for the rare case a route segment genuinely suspends for a while. Because production Next.js links are prefetched, fast route changes may never suspend and therefore may not display that fallback. `AppNavigationLoader` (`src/components/feedback/app-navigation-loader.tsx`) adds a short RF-branded transition on valid internal link navigation so users receive consistent feedback without disabling prefetching or delaying data requests. External/new-tab/download/same-page interactions are excluded, and the app-wide reduced-motion policy applies.
...
The status pill was also simplified: the `animate-ping` ring around the icon (semantically closer to "notification" than "loading" in most UI conventions) is replaced with a spinning ring — the same visual language `RfLoadingScreen`'s full-boundary fallback now also uses, so the app has one consistent "loading" identity instead of two. The second line of body copy ("Your current page will stay visible") was dropped — it explained an implementation detail rather than telling the user anything useful, and shown on every single navigation click it added visual weight without value.

...
- **Permissions** - 146 permission keys are seeded (140 module-specific keys across sixteen module prefixes, plus 6 shared dashboard, organization, AI-assistant, and audit-view/export keys). See `src/lib/auth/permissions.ts` for the current list. **Do not reuse a permission across modules.** Accounting separates period management and journal reversal from its other operational permissions. Hotel, School, Hostel, Pharmacy, and Hospital retain their own module-prefixed permission families.

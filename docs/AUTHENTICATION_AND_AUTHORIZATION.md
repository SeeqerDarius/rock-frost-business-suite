...
**Status: authentication, role/module-level authorization, and action-level permissions across all sixteen modules, including Hotel, School, and Hostel, are real and enforced.**
...
- **Authenticator-based two-factor authentication (TOTP):** every platform administrator and organization user can enroll from Account > Security. Setup requires the current password; enabling or disabling 2FA revokes existing sessions. Enrolled accounts must provide a six-digit TOTP code after their password on every login, and invalid codes participate in the existing login lockout policy. Secrets are encrypted at rest with AES-256-GCM by `src/lib/auth/totp.ts`, using `TWO_FACTOR_ENCRYPTION_KEY` when configured and otherwise the deployment's `NEXTAUTH_SECRET`. The optional dedicated key should be set before production enrollment and must be retained while enrolled secrets need to remain readable.
...
- **Permissions**: 159 permission keys are seeded across sixteen module prefixes and shared platform capabilities. See `src/lib/auth/permissions.ts` for the current list. **Do not reuse a permission across modules.** Inventory separates stock-count entry from stock-count approval. Procurement separates request entry and approval, order management, receiving, invoice entry, invoice approval, and supplier-payment entry so maker-checker review can be enforced. Accounting separates customer receipt entry from invoice creation. Point of Sale separates returns and variance approval from ordinary selling. Hospital separates laboratory and imaging entry from verification. During production seeding, legacy tenant-role grants for `hospital.lab.manage` and `hospital.imaging.manage` are migrated to the corresponding entry permission only. Verification is never granted implicitly.
...
- **Data-level scoping within modules**: Installment field staff are restricted to their own assigned customers, accounts, payments, and credits. Fleet drivers are restricted to assigned vehicles, their own collection submissions, and maintenance for those vehicles. Vehicle Owner users are restricted to vehicles linked to their owner profile. These rules are enforced in server queries and actions, not only in navigation.
...
3. **Permission-aware page-level guards**: `/app/platform/*`, `/app/administration`, `/app/organization`, `/app/fleet`, and `/app/installment` all redirect or block server-side for a user who reaches the URL directly without the right access. Fleet Driver has a dedicated assigned-vehicle workspace without `fleet.view`. Vehicle Owner uses the linked portfolio and maintenance approval pages without organization-wide Fleet access.

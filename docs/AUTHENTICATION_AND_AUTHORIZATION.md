...
- **Permissions** — 150 permission keys are seeded (144 module-specific keys across sixteen module prefixes, plus 6 shared dashboard, organization, AI-assistant, and audit-view/export keys). See `src/lib/auth/permissions.ts` for the current list. **Do not reuse a permission across modules.** Inventory separates stock-count entry from stock-count approval. Procurement separates request entry and approval, order management, receiving, invoice entry, and invoice approval so maker-checker review can be enforced.

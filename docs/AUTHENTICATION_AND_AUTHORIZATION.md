...
- **Permissions**: 157 permission keys are seeded across sixteen module prefixes and shared platform capabilities. See `src/lib/auth/permissions.ts` for the current list. **Do not reuse a permission across modules.** Inventory separates stock-count entry from stock-count approval. Procurement separates request entry and approval, order management, receiving, invoice entry, and invoice approval so maker-checker review can be enforced. Point of Sale separates returns and variance approval from ordinary selling. Hospital separates laboratory and imaging entry from verification. During production seeding, legacy tenant-role grants for `hospital.lab.manage` and `hospital.imaging.manage` are migrated to the corresponding entry permission only. Verification is never granted implicitly.

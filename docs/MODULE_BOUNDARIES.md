...
Accounting journal integrations persist the canonical source module and an authoritative branch when the source workflow can resolve one. The shared posting boundary rejects a branch owned by another organization. It leaves the dimension empty when the source is branchless or spans multiple branches rather than inferring one from the current user.

**A module-specific staff picker (a class teacher, a hostel warden, a fleet driver/owner/mechanic) must filter to organization members who actually hold that module's permission, never every active organization member.** Fleet's `listAssignableDriverUsers`/`listAssignableOwnerUsers`/`listAssignableMechanicUsers` (`src/modules/fleet/service.ts`) established this: filter the `organizationMember` query itself, either by an exact permission key (`role: { rolePermissions: { some: { permission: { key: PERMISSIONS.FLEET_DRIVER_SELF_SERVICE } } } }`) when the role has one canonical marker permission, or by the module's own permission prefix (`role: { rolePermissions: { some: { permission: { key: { startsWith: "school." } } } } }`) when it doesn't. School's `listAssignableTeacherUsers`, Hostel's `listAssignableHostelUsers`, and Installment's `listAssignableStaffUsers` (originally unfiltered — every active member, including e.g. a Fleet driver, showed up in the teacher/warden/staff-login picker) were fixed to the same pattern, School and Hostel in one release and Installment in a follow-up audit of the same bug class; see `OPERATOR_HANDOFF.md`'s corresponding entries. Installment's case carries extra weight: the linked user gains real `resolveInstallmentAccessScope()` self-service data access over their own customers and accounts, so an unfiltered picker there wasn't just a confusing dropdown, it was an unintended access grant. This is a different case from CRM's/Projects's/HR's own `listAssignableUsers`/`listActiveMembers`/`listContractTemplateResponsibleCandidates` — those pick a genuinely cross-functional "owner" or "responsible party" for a lead, task, or contract, a concept that isn't scoped to one module's staff by design, so they deliberately stay unfiltered. Don't "fix" those to match; a new module-specific staff role picker should match the filtered pattern instead.

## Module-local team management

CRM, Inventory, Procurement, Accounting, Payroll, Analytics, POS, Projects, Hotel, Pharmacy, and Hospital share one membership-backed Team workflow at `/app/[moduleKey]/staff`. It lists only the fixed system-role family for the selected module and permits invitations, role changes, activation, and suspension only after both module access and the configured manager permission pass. Every lookup and mutation includes the current organization ID, platform identities are rejected, subscription seats are enforced, self-suspension is blocked, and mutations are audited. Activating a member synchronizes the shared identity into HR when available, so the Team page never creates a competing employee model.

Fleet, Installment, Hostel, School, and HR deliberately remain outside that generic route. Their existing staff surfaces carry specialist assignments or authoritative workforce data, including vehicles, customers, buildings, classes, or employee records. Those workflows must keep their own linking invariants instead of being flattened into generic membership management.

...
**Status: implemented and enforced.** In-app support chat between tenant organizations and Rock Frost platform operators. Migration `20260813040000_add_support_messaging`; per-user conversations and the organization admin inbox added in `20260830090000_support_per_user_conversations`.
...
- `isPlatformOnline()` checks presence across every active Super Admin (`role: { name: "Super Admin", isSystem: true, organizationId: null }`) — the tenant sees "online" the moment any platform operator has an open, visible support tab, regardless of which conversation that operator is actually looking at. This is also what currently gates whether the AI assistant replies (see "AI-assisted first response" below) — a known imprecision, tracked in "Known gaps."
...
- **No platform operator is currently online** (`support.isPlatformOnline()`) — the same presence check the tenant's own "Online"/"Offline" indicator already uses. This is the deliberate "hand off to a human" behavior: when a human is present, the human answers; when none is, the AI does, and a human can still jump into the same thread at any time afterward. Because this check is platform-wide rather than scoped to the one conversation an operator is actually viewing, one operator idly viewing any conversation currently suppresses AI replies everywhere for up to 45 seconds — see "Known gaps."
...
- `isPlatformOnline()` — the check that gates AI eligibility — is platform-wide rather than scoped to the specific conversation an operator is viewing. An operator idly viewing any one tenant's conversation currently suppresses the AI assistant's replies for every organization, not just the one they're looking at, for up to 45 seconds after their last heartbeat. Tracked as a known imprecision to fix with a conversation-scoped presence check.

...
- An administrator with `org.settings.manage` can export all module records or a single module as JSON.
...
- Restore accepts only a backup whose organization ID and tenant code exactly match the active organization. Every row is revalidated for that same organization.

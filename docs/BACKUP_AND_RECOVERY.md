...
- Accounting, Analytics, CRM, Fleet, Hospital, Hotel, HR, Installment, Inventory, Payroll, POS, Procurement, Projects, and School are valid scopes. Analytics currently owns no independent fact tables, so its scoped export is intentionally empty. Hospital (branch `agent/claude-hospital-production`, not yet merged) required no bespoke backup code — it was added to `src/lib/backup/scopes.ts` and the `Hospital`-prefix entry in `src/lib/backup/tenant-backup.ts`'s model map, and every `Hospital*` table with `organizationId`/`id` is then auto-discovered by the existing generic export/restore logic.

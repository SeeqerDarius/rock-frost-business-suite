...
**The sidebar renders every enabled module at once, with the current one's own pages shown directly** (`ModuleSectionsNav`, in `src/components/navigation/module-sections-nav.tsx`), driven by `platform/modules/full-navigation.tsx`'s `getFullModuleNavigation(tenant)`. The module the current page actually belongs to shows its own real, permission-filtered page list right there in the sidebar, no click required; every other enabled module is a single flat link grouped under an "Other modules" heading - clicking one navigates into it, where its own layout takes over and this component then shows its pages in place of the module you left. This is derived fresh from the pathname on every render, not stored as state, so it can never drift out of sync with the real current route. (An earlier revision of this component rendered every module as a true click-to-expand accordion, one open at a time; that was reverted back to this flatter, always-visible design.)
...
Each business-module `layout.tsx` (plus the Overview layout) passes `moduleSections={getFullModuleNavigation(tenant)}` to `AppShell`. Inside a module, `AppShell` shows only `ModuleSectionsNav` (the current module's pages show directly, since it matches the current path); at the Overview/workspace scope it shows the flat workspace navigation list plus `ModuleSectionsNav` beneath it (where every module reads as "other," since no business-module route is current). The platform-operator scope (`app/platform/*`) never receives `moduleSections` - platform operators don't browse tenant business modules. Suppressed by the same `showModuleLauncher`/driver-role checks that already hide the header module launcher for locked-down roles (e.g. Fleet's Driver role - see `FLEET_MODULE_IMPLEMENTATION.md`).

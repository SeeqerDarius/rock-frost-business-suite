...
**The sidebar shows exactly one module at a time: whichever one the current page belongs to.** Each module's `layout.tsx` computes its own permission-filtered page list and passes it to `AppShell` as the `navigation` prop; `AppShell` renders it as a plain list, nothing else. There is no cross-module list, accordion, or "other modules" section inside the sidebar itself - switching modules happens entirely through the header's `ModuleLauncher` (the module-switcher dialog), which reads from `platform/modules/registry.ts` independently of the sidebar. (Two later revisions briefly added a sidebar-level view of every enabled module at once - first a flat "other modules" list, then a click-to-expand accordion, driven by a `full-navigation.tsx`/`moduleSections` layer - before both were reverted back to this original, single-module design; that layer no longer exists in the codebase.)

This still respects the same isolation principle above - only a module's own layout has the tenant/role context to filter its pages correctly. Fleet and Installment keep that filtering logic in a dedicated `navigation-access.ts` file, separate from their plain `navigation.tsx` array, for a structural reason: `registry.ts` imports `navigation.tsx` for every module (for the catalogue/launcher), and `AppShell` (a Client Component) imports `registry.ts` - so anything `navigation.tsx` imports ends up in the client bundle. `@/lib/auth/permissions` starts with `import "server-only"`, so a tenant-aware, permission-checking function must never live in `navigation.tsx` itself (confirmed by a real Turbopack build failure during development); it lives in `navigation-access.ts` instead, imported only by that module's own `layout.tsx`.
...
- **`requireCurrentTenant()` throws on a null tenant - most call sites should not use it directly.** It exists for the rare case where a null tenant genuinely cannot happen (the caller already sits behind a layout guard with no independent tenant lookup of its own) or where an unhandled throw is the correct behavior. A page or layout that runs its *own* tenant lookup (most of them do, since `getCurrentTenant()` re-queries the database rather than trusting anything cached) should call `getCurrentTenant()` and degrade gracefully on null instead - a transient database hiccup between the root layout's own check and a nested layout's or page's independent one is a real, observed production failure mode (confirmed 2026-08-29, `src/app/app/(overview)/dashboard/page.tsx` and `src/app/app/fleet/layout.tsx` both hit it), and it must never surface as a raw 500 for what is, seconds later, a perfectly valid session. `dashboard/page.tsx` renders the same "no organization access" message the root layout uses; a nested module layout with no natural fallback UI of its own (e.g. `fleet/layout.tsx`) redirects to `/login`, which both recovers cleanly from a transient failure on retry and correctly signs out someone whose access was genuinely revoked. A wider audit of every remaining `requireCurrentTenant()` call site for the same gap is tracked separately, not yet complete.
...
- **A dynamic route segment under `/app` needs its own `layout.tsx` just like a static module folder does - Next.js does not fall back to an ancestor's `AppShell`.** The shared Team directory at `/app/<moduleKey>/staff` (`src/modules/staff/module-team-config.ts`) shipped with a `page.tsx` but no `src/app/app/[moduleKey]/layout.tsx`, so it rendered directly under the bare authenticated `src/app/app/layout.tsx` (auth/tenant/providers only, no sidebar) instead of any module's `AppShell` - losing the sidebar entirely and stretching full-width, for all eleven modules using that shared page (confirmed 2026-09-04 from a user report against `/app/pos/staff`). Fixed by adding `src/app/app/[moduleKey]/layout.tsx`, which resolves `getModuleTeamConfig(moduleKey)` and computes that specific module's `sectionLabel`/`navigation` the same way its own real `layout.tsx` does (reusing the same helper functions for Inventory/Procurement's and Payroll's dynamic navigation, so the Team page's sidebar never drifts from that module's real pages) before rendering `AppShell`. A future dynamic route added anywhere under `/app` needs the same treatment; a page.tsx alone is not enough.
# Offline browser boundary

The installable browser application uses a public versioned service-worker shell and tenant data in IndexedDB. Authenticated HTML, React Server Component output, and API responses are never written to shared CacheStorage. IndexedDB records are partitioned by organization, user, module, and registered device, with explicit expiry and purge behavior.

Offline writes enter an append-only client outbox and a tenant-scoped server idempotency ledger. Signed synchronization reuses authoritative module services instead of writing module tables directly. Protected version mismatches create explicit conflict records. Safe drafts are stored separately and cannot become posted, paid, verified, approved, reconciled, dispensed, fulfilled, or clinically finalized. See `docs/OFFLINE_PWA.md` and `docs/OFFLINE_CAPABILITY_MATRIX.md`.

...
- **Support messaging** is cross-cutting organization-scope + platform-scope infrastructure (`/app/support`, `/app/platform/support`), deliberately not a business module — no registry entry, no module-prefix permission, no backup-export inclusion. Heartbeat-based presence, polling-based delivery, no email anywhere in the feature. See `docs/SUPPORT_MESSAGING.md`.

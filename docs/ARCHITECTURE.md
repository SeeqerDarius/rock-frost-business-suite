...
**The sidebar also lists every other enabled module as a single collapsed link** (`AppShell`'s `otherModuleNavItems`, built by `platform/modules/enabled-module-tiles.ts`'s `getEnabledModuleTiles()`), beneath the current module's own expanded page list — so an org with several modules active sees all of them at once, and clicking one navigates into it, where *that* module's own layout takes over and expands its own permission-filtered pages. This is deliberately a link, never that other module's actual sub-items: only a module's own layout has the tenant/role context to filter its pages correctly, and pre-computing that for every module inside the shared shell would either duplicate every module's permission logic in a second place (able to drift from the real one) or leak page links a role can't actually use. Suppressed by the same `showModuleLauncher` flag that already hides the header module launcher for locked-down roles (e.g. Fleet's Driver role - see `FLEET_MODULE_IMPLEMENTATION.md`).

The sidebar's collapse/expand toggle sits at the top, to the left of the workspace logo (a plain hamburger icon button) — not the bottom "Collapse sidebar" button an earlier version had. Same `toggleSidebar()`/`localStorage` mechanism either way; only the control's position and icon changed, to match a layout the user asked to mirror.

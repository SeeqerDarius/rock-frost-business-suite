...
- The Driver Workspace is a compact "at a glance" summary (assigned vehicle, balance, what's left to pay) followed by two tabs - "Record a completed payment" (online-pay buttons, the manual payment form, revenue trend, and payment history) and "Maintenance" (report an issue and the driver's own report history) - rather than one long scrolling page mixing read-only status with action forms

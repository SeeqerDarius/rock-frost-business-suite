...
The portal's one new self-service action so far, `acceptMaintenanceAssignment()`, lets a mechanic record their own scheduled repair date on a request assigned to them (logging a `REPAIR_SCHEDULED` event) without yet advancing `progressStatus` - the `ASSIGNED`/`SCHEDULED` states that action conceptually belongs to don't exist in `FleetMaintenanceProgressStatus` yet. A later progress-status enum expansion activates the real state transition; this action itself does not need to change when that happens.

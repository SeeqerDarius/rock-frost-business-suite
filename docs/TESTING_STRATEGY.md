...
**Module mocks in the integration suite belong in `setupFiles`, never in one test file.** Because the suite runs `isolate: false` in a single fork, all 57 files share one module registry, so a `vi.mock("@/some/module", ...)` written inside a test file only takes effect if that file happens to be the first to pull that module into the registry. Vitest's file order is not something a test should depend on. This bit us for real: the Paystack HTTP mock lived inside `test/integration/settlement-activation-lifecycle.test.ts`, but `test/integration/settlement-reconciliation-retry.test.ts` imports `@/lib/payments/operational`, which imports the same barrel unmocked. Locally the alphabetical order put the mocking file first and everything passed; in CI it did not, the real client ran, and the suite failed with `Paystack is not configured (PAYSTACK_SECRET_KEY unset)` for months while being misread as a missing CI secret. It was never a secret problem: `paystackRequest()` throws on an unset key *before* it calls `fetch`, so adding a placeholder key would have made CI start issuing real requests to `api.paystack.co`. **No automated test may reach a third-party API, with or without a working credential.** The mock now lives in `test/integration/setup/paystack-http.ts`. Reproduce this class of bug with `--sequence.shuffle.files --sequence.seed=<n>` before trusting that a new mock is order-independent. `isolate: true` is not an available escape hatch: six files depend on the shared registry and fail under it.

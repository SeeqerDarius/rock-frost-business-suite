...
**2026-08-20 posting-foundation upgrade.** Journal entries now receive immutable organization-scoped posting numbers. Source modules have one idempotent posting contract keyed by organization, source type, source id, and posting purpose. Accounting periods can be opened, closed, and reopened; a closed period rejects every posting dated within it. Corrections use a full compensating reversal entry while retaining the original journal. The new controls are separated into `accounting.periods.manage` and `accounting.journals.reverse` permissions. See `docs/ACCOUNTING_MODULE.md` for the integration boundary and remaining scope.

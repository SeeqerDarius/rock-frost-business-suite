...
**Configuration** consolidates HR's master-data lookups as stacked Card sections — deliberately **not** one route per lookup (Odoo's own Configuration menu shows 8 separate items, but this codebase already has its own precedent for "several small related settings" living in one page as separate sections). It has **Skills** (see above) plus 6 more sections for the remaining simple named lookups: Employee Types, Work Locations, Departure Reasons, Working Schedules, Time Types, Job Positions. Each is `{id, organizationId, name, createdAt}` (Work Locations add `locationType`: `OFFICE`/`REMOTE`/`HYBRID`), shipped as real, functioning CRUD with no deeper logic: no calendar engine for Working Schedules, no link into the termination workflow or anywhere else, per an explicit scoping decision with the user. These 6 sections render through one shared component, `src/app/app/hr/configuration/named-lookup-section.tsx` (a name list with a per-row delete button, plus an inline add form), rather than 6 hand-duplicated Card sections; the underlying Prisma models and service functions (`src/modules/hr/service.ts`) and their Server Actions (`src/app/app/hr/configuration/actions.ts`) stay in their own files and redirect back to `/app/hr/settings` after a mutation — only the page-level presentation moved.

**Job Position is one lookup with a real consumer.** `HrJobPosition.name` values seed an HTML `<datalist>` on the employee form's `jobTitle` field (`src/app/app/hr/employees/employee-fields.tsx`) — a plain native combobox (`<input list="...">`), not a new client component: pick a suggested position or type a new one exactly as before. `jobTitle` itself stays the free-text column of record on `HrEmployee`; there is no foreign key and no migration risk to existing data.

**Contract Templates is the one lookup that graduated into a full editable form**, matching Odoo's own Contract Template rather than shipping as a bare name. `HrContractTemplate` carries: `jobPositionId` (→ `HrJobPosition`), `department` (free text, matching `HrEmployee.department`'s own convention), `hrResponsibleId` (→ `User`, picked from the organization's active members — the same "active members with a login" shape CRM's `listActiveMembers()` already uses for owner pickers, exposed here as `listContractTemplateResponsibleCandidates()`), `employeeTypeId` (→ `HrEmployeeType`), and a Salary Information section: `wageType` (`FIXED`/`HOURLY`, free text, no new enum), `payFrequency` (`MONTHLY`/`BIWEEKLY`/`WEEKLY`, reusing `PayrollCompensation.payFrequency`'s own free-text convention and values), `wage` (`Decimal(12,2)`), `excludedFromPayRuns` (boolean — the one piece of real, actionable meaning behind Odoo's "Pay Category" field; a full Pay Category catalog was skipped since nothing in this codebase's Payroll module has that concept yet, and adding one with no consumer would be the same "dead weight" mistake avoided for Job Position in Phase J), and `workingScheduleId` (→ `HrWorkingSchedule`). All 4 foreign keys are nullable with `onDelete: SetNull` and validated per-organization on every create/update (`requireContractTemplateForeignKeys()` in `src/modules/hr/service.ts`) — deleting a referenced Job Position, Employee Type, or Working Schedule just clears the template's reference rather than blocking the delete or cascading it away. This is still **not** wired into Payroll's own `PayrollCompensation`, which stays the real source of truth once an employee is actually hired — a template is only what a future contract should start from, edited through its own Dialog-based form (`src/app/app/hr/configuration/contract-template-fields.tsx`) rather than the shared `NamedLookupSection`.

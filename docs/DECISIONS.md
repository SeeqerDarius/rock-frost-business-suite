...
**Why:** The wizard needs to *display* the seven-check readiness report the moment an administrator opens the readiness step - that's a GET request, and a GET request must never write to the database as a side effect. Without this, simply loading the readiness page while the account happened to be `VERIFIED` and every other check passed would silently flip the profile to `ACTIVE`, with no explicit action taken by anyone. Adding a `commit` flag rather than splitting the function in two kept the seven-check computation in exactly one place - a preview and a commit computing the checks differently would be a correctness risk (an admin could see "READY" in preview, then have the real commit disagree because the two code paths drifted).

**Also decided in this phase - bank name is never trusted from the client.** The old single-step settlement form collected "Bank name" as free text alongside a free-text "Paystack bank code," so nothing stopped the two from disagreeing, and nothing validated the code was real. The wizard's account-setup step replaces both with a single bank picker built from a live Paystack bank list (`listPaystackBanks`, previously implemented but never called from any route); the Server Action re-derives the bank's name from that same live list server-side rather than accepting whatever the client submits, so a forged `bankCode` that doesn't match a real bank is rejected outright instead of silently accepted with a fabricated name.

**Also decided: the old single-step settlement form and its `saveSettlementProfile()`/`saveSettlementAccount()` functions are deleted, not deprecated-and-kept.** Once the wizard fully replaced their only call site, they had zero remaining callers - per this codebase's own standing convention against backwards-compatibility shims for code that's genuinely unused, they were removed outright rather than left as dead exports.

## 2026-08-29 - Online Collections activation, Phase B1: a real SettlementProfileStatus lifecycle, replacing the always-ACTIVE shortcut

**Decision:** `src/lib/payments/operational.ts`'s single `saveSettlementProfile()` (which always wrote `status: "ACTIVE"` on success) is split into `initiateSettlementProfile()` (writes `PENDING`), `confirmSettlementBeneficiary()` (`PENDING` -> `VERIFIED`), and `runSettlementReadinessCheck()` (only on a full pass, and only from `VERIFIED`, promotes to `ACTIVE`). `saveSettlementProfile()` itself is kept as a deprecated wrapper around `initiateSettlementProfile()` so the existing single-step Organization Settings form keeps working unchanged through this phase and the next - it now creates at `PENDING` instead of `ACTIVE`.

**Why:** `SettlementProfileStatus` (`PENDING`/`VERIFIED`/`ACTIVE`/`SUSPENDED`/`FAILED`) has existed in the schema since the settlement feature first shipped, but no code path ever used more than two of its five values - every save jumped straight to `ACTIVE` or threw. The guided "Online Collections" activation flow this phase starts building needs the lifecycle to be real: an administrator should see "verification required" and "under review" as honest intermediate states, not have every save silently claim the account is already fully active. The user's 6 requested states (Not started/Verification required/Under review/Active/Restricted/Failed) map onto the existing 5 enum values with no migration: "Not started" is simply the absence of a `SettlementProfile` row, and "Restricted" is a presentation-layer rename of `SUSPENDED` - there was no requirement that would justify a 6th enum value.

**A deliberate interim behavior change, accepted as part of the approved phased plan:** updating bank details through the current form (or via `initiateSettlementProfile()` directly) now resets an already-`ACTIVE` profile to `PENDING` and disables online collections until the account is re-verified and a full readiness check passes again - even for an org that was already collecting payments live. This is intentional: a profile must never stay `ACTIVE` against bank details Rock Frost hasn't re-verified. The real UI to carry an admin through re-verification without a confusing gap doesn't exist until Phase B3's guided wizard; until then, an admin who edits their bank details on the existing single-step form will see their collections pause. This is disclosed here and in `OPERATOR_HANDOFF.md` rather than silently shipped.

**Safety detail**: `runSettlementReadinessCheck()` never auto-reactivates a `SUSPENDED` or `FAILED` profile back to `ACTIVE`, even if all seven readiness checks mechanically pass (a `SUSPENDED` profile still reports its account as "verified," since the underlying identity check once passed) - only a fresh `VERIFIED` profile can be promoted, and lifting an administrative restriction is left as a deliberate, separate action.
